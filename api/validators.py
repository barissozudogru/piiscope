...
def validate_json_data(data: str | dict) -> dict[str, Any]:
...
def validate_profile_definition(definition: dict[str, Any]) -> dict[str, Any]:
    ...
    for pattern_name, pattern_data in patterns.items():
        ...
        try:
            ...
            raise ValidationError(f"Invalid regex in pattern '{pattern_name}': {str(e)}") from e
...
def validate_pydantic_model(model_class: type[BaseModel], data: dict[str, Any]) -> BaseModel:
    ...
    try:
        ...
        raise ValidationError(f"Validation errors: {'; '.join(error_messages)}") from None
